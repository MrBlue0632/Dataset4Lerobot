...
def get_total_episodes(input_dir):
    ...
    # 返回最大的episode编号
    if min(episode_numbers)==0:
        total_episodes = max(episode_numbers)+1
    else:
...
def process_image(image, width=128, height=128):
    """处理单个图像帧
    ...
    Returns:
        处理后的图像，统一为(128, 128, 3)大小的numpy数组
    """
    try:
...
def convert_episode(input_dir, output_dir, episode_index):
    ...
    try:
        ...
        #snapshot_file = input_dir / f"lerobot_episode_{episode_index+1}.parquet"
        snapshot_file = input_dir / f"lerobot_episode_{episode_index}.parquet"
...
def create_video(images, output_file, width=128, height=128, fps=30):
    """创建视频文件，优化编码过程"""
    try:
...
def create_info_json(output_dir, total_episodes, total_frames, image_shapes, fps=30, input_dir=None):
    ...
    info["features"]["observation.state"] = {
        "dtype": "float32",
        "shape": [8],  # xarm6有6个关节加1个夹爪
        "names": {
            "joints": ["joint_0", "joint_1", "joint_2", "joint_3", "joint_4", "joint_5", "joint_6","gripper"]
        }
    }
...
def create_episodes_jsonl(output_dir, total_episodes, input_dir):
    ...
    with open(output_dir / "meta" / "episodes.jsonl", "w") as f:
        for i in range(total_episodes):
            # 读取原始数据文件以获取语言指导
            snapshot_file = input_dir / f"lerobot_episode_{i}.parquet"
...
def process_episode_stats(args):
    ...
    """
    try:
        i, output_dir, input_dir, _ = args
    except ValueError:
        i, output_dir, input_dir = args
    ...
    try:
        ...
        for col in image_columns:
            try:
                ...
                # 预分配内存
                images = np.zeros((sample_size, 128, 128, 3), dtype=np.float32)
                valid_count = 0
                ...
                for idx, sample_idx in enumerate(sample_indices):
                    ...
                    if isinstance(img_data, bytes):
                        ...
                        img = cv2.resize(img_data, (128, 128), interpolation=cv2.INTER_AREA)
...
def create_episodes_stats_jsonl(output_dir, total_episodes, input_dir):
    ...
    # 准备并行处理的参数
    args_list = [(i, output_dir, input_dir) for i in range(total_episodes)]
...
def main():
    ...
    parser.add_argument("--width", type=int, default=128, help="处理后的图像宽度")
    parser.add_argument("--height", type=int, default=128, help="处理后的图像高度")
    ...
    # 获取图像列和形状信息
    snapshot_file = input_dir / "lerobot_episode_1.parquet"
    if not snapshot_file.exists():
